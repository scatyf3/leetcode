...
| | | **不单独记** —— 由每题的熟练度 `familiarity` (L1–L4) 算出来，见下 |
...
- 底下按 pattern group 列题。方块里写的是**原始的 L**，方块的**颜色**是算出来的 S —— 两个都看得见。
  点方块循环 `· → L4 → L3 → L2 → L1`，写回该题 `meta.json`；和详情页那个下拉是同一个字段。
...
深度轴**没有自己的存储**。真相只有两处，都在每题的 `meta.json` 里：

| 存的 | 是什么 | 在哪改 |
|------|--------|--------|
| `familiarity` 1–4 | 熟练度 L1–L4 | 详情页「熟练度」下拉，或坐标系里点方块 |
| `explained` true | 英语 60 秒讲得清 | 坐标系里点「讲」 |

映射：

```
L1 已经熟悉            ─┐
                        ├─→ S2 写得对（OA 门槛）  ─── ＋ explained ──→ S3 讲得清
L2 思路会 · 细节易写错 ─┘
...
- **「讲」只在 L1/L2 的题上出现。** 写不对的题不该因为讲得出就混进 S2 那一列 ——
  而 S2 正是你判断「能不能去做 OA」的那一列，虚高的代价最大。这样 S3 ⊂ S2 ⊂ S1 恒成立。
- **一处真相。** 早先版本把掌握度另存在 `dashboard/progress.json` 里，结果同一件事有两个记录点，
  会出现「熟练度 L2 但坐标系标了 S1」这种自相矛盾的状态。现在算出来，不可能矛盾。

题单 150 题里大半还没有文件夹 —— 那些题没有 `meta.json`，也就自动算作「还没到 S1」，正是想要的语义。
...
| PUT  | `/api/problems/{id}/meta`| 也吃 `{familiarity}` / `{explained}`，坐标系就用这个 |
